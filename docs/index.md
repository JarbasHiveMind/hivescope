...
| `assert_session_inbound_preserved(master, satellite, expected_session)` | BRIDGE-1 §4.1 (inbound): the satellite's session CONTENTS (lang, location, etc.) land unchanged in the bus-injected message's `context.session`. Does not accept `session_id` — see `assert_session_id_natted` |
| `assert_session_id_natted(master, satellite, declared_id, admin=False)` | BRIDGE-1 §4: a non-admin's declared `session_id` is NATted per connection to `f"{conn_nonce}:{declared_id}"` before it reaches the bus; an admin's is stamped raw. Reads the live `conn_nonce` off the master connection and asserts the exact NATted id |
| `assert_sessions_isolated(master, satellite, declared_ids)` | BRIDGE-1 §4: several declared session ids sent over one connection (a relay, or a per-call bridge) each produce a distinct Layer-1 session sharing one nonce prefix — no cross-talk between multiplexed sessions |
| `assert_session_contents_merged_over_baseline(master, satellite, expected_baseline)` | BRIDGE-1 §4: a thin message that omits a field (e.g. only `session_id`) keeps the connection's HELLO-established baseline for that field instead of it being overwritten by a fresh default |
